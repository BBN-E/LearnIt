...
sub extract_mappings {
	...
	foreach my $batch_list (@batch_source_lists) {
		...
		my $extract_thread_job = run_on_queue("extract-${batch_name}-".$frequency_combination,
				"50G",
				 make_java_command("75G", $params->{"learnit_root"}."/neolearnit/target/appassembler/bin/", "LabelExtractor","$PARAMS $TARGET $batch_list $output_file.before_filter"));
...
sub create_aggregate_mappings_file{
	...
	run_on_queue_now_local("create-aggregate-mappings-".$frequency_combination,
		make_java_command("128G", $params->{"learnit_root"}."/neolearnit/target/appassembler/bin/", "MergeMappings","$PARAMS $mappings_lists_dir $output_mappings_file.before_filter"));
