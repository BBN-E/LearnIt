...
# our $QUEUE = "afrl";
# our $QUEUE = "r200s";
our $QUEUE_PRIORITY = 7;

my ($expt_dir, $exp) = startjobs();
max_jobs(500);  # Max number of jobs to run at once

my $MAVEN_BIN = "/opt/apache-maven-3.0.4/bin/mvn";

my $job_prefix = "gigaword_1.5M-train-LABELING_INSTANCE.sampled500.v4";
my $learnit_repo="/nfs/ld100/u10/bmin/repositories/learnit.master";
my $params="$learnit_repo/params/learnit/runs/gigaword_1.5M.params";
my $min_freq_seeds="5";
my $MAX_INSTANCES_PER_SEED=10;
my $NEGATIVE_SAMPLING_RATIO=0.4;

my $initial_extractor_list="/nfs/ld100/u10/bmin/repositories/learnit.master/resources/learnit_extractors.list";
# my $initial_mappings_list="$learnit_repo/resources/mappings_for_nre_train-n50.list"; # only the first 50 out of 1450 mappings
my $initial_mappings_list="$learnit_repo/resources/mappings_for_nre_train.list";
...
#
#        my $dir_extractors="${expt_dir}/extractors-${extractor_name}";
#        system "mkdir $dir_extractors";
#
...
#
#		my $input_extractor = $extractor;
#                my $output_extractor = "$dir_extractors/extractor-${mappings_name}";
#		my $iteration = 1;
#
#		run_on_queue("${job_prefix}/GeneratePairsFromPatterns-${extractor_name}-${mappings_name}",
#				"4G",
#				make_java_command("4G", "$learnit_repo/neolearnit/target/appassembler/bin/", "GeneratePairsFromPatterns", "$params $input_extractor $output_extractor $mappings 1"));
#	}
#
#	dojobs();
#	
#        my $list_output_extractors = "${dir_list_output_extractors}/list_${extractor_name}";
#	my $final_extractor = "${dir_final_extractors}/${extractor_name}.json";
#	system "find $dir_extractors/extractor-* > ${list_output_extractors}";
#	run_on_queue("${job_prefix}/SelectPairs-${extractor_name}",
#			"4G",
#			make_java_command("4G", "$learnit_repo/neolearnit/target/appassembler/bin/", "SelectPairs", "$params $list_output_extractors $final_extractor $min_freq_seeds 1"));
#	dojobs();
#}
...
while (my $extractor = <$fh>) {
	...
	# generate NRE data
		...
		my $output_file_prefix = "${dir_opennre_intermediate}/opennre-${extractor_name}-${mappings_name}";
                ...
                my $output_json="${output_file_prefix}/data-shrink.json";
		run_on_queue_with_prev_job("${job_prefix}/SampleNegative-${extractor_name}-${mappings_name}",
				"4G",
				"${job_prefix}/GenerateTrainingDataFromSeedsForOpenNRE-${extractor_name}-${mappings_name}",
				"python $learnit_repo/neolearnit/src/main/python/sample_negatives.py $input_json $output_json");
        }
}
...
system "find ${dir_opennre_intermediate}/ | grep data-shrink.json > ${list_input_json}";
my $output_json="${dir_opennre_final}/final.json";

run_on_queue("${job_prefix}/MergeJsonFile",
		"96G",
		"python $learnit_repo/neolearnit/src/main/python/merge_json_file.py $list_input_json $output_json");
