...
public class AmtRelationTaskSampler implements MappingLabeler {
...
//            System.out.println("Killed: "+learnitPattern.toIDString());
    ...
    @Override
    public Mappings LabelMappings(Mappings mappings) {
        mappings = new InstanceIdentifierFilter().makeFiltered(mappings);
        mappings = new SingleDirectionFilter().makeFiltered(mappings);
        ...
        for (LearnitPattern learnitPattern : new HashSet<>(mappings.getAllPatterns())) {
            if (isValidPattern(learnitPattern))
                countingMap.add(new Pair<>(learnitPattern, new HashSet<>(mappings.getInstancesForPattern(learnitPattern))));
        }
...
//        }
        ...
        for (Pair<LearnItObservation, Set<InstanceIdentifier>> countItem : countingMap) {
            ...
            for (int i = 0; i < Math.min(filteredInstanceIdentifier.size(), MAX_SENTENCES_PER_PATTERN_TO_ADD); ++i) {
                inMemoryAnnotationStorage.addAnnotation(filteredInstanceIdentifier.get(i), new LabelPattern(NEGATIVE_TYPE, Annotation.FrozenState.NO_FROZEN));
            }
        }
    ...
    public static void main(String[] args) throws Exception {
        ...
        Mappings outputMappings = AmtRelationTaskSampler.LabelMappings(Mappings.deserialize(new File(autogeneratedMappingsPath), true));
