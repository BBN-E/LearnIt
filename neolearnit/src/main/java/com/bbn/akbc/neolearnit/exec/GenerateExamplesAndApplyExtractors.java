...
public class GenerateExamplesAndApplyExtractors {
    ...
    public static void main(String[] args) throws IOException {
        ...
        final String patternOutputFile = LearnItConfig.get("patternOutputFile");
        ...
        final List<TargetAndScoreTables> extractors = loadExtractors();             // load relation extractors
        ...
        for(final TargetAndScoreTables extractor : extractors) {
            if(extractor.getTarget().getName().indexOf("unary_")==-1) {             // i.e. this is a binary relation extractor
                ...
                loader.load(candidates);    // if a relation mention satisfies basic constraints, then apply the various observers

                // the various Observers have been applied to obtain instances. Now apply user-defined patterns to filters instances
                final Multimap<MatchInfo, LearnitPattern> matchingRelationMentions = module.getExtractionResult(extractor);
                for(final MatchInfo eg : matchingRelationMentions.keySet()) {
                    final String egString = printRelationMention(eg, extractor.getTarget().getName());
                    mentionOutputStrings.add(egString);

                    for(final LearnitPattern pattern : matchingRelationMentions.get(eg)) {
                      patternOutputStrings.add(eg.uniqueIdentifier() + "\t" + pattern.toString());
                    }
                }
    ...
    private static String printRelationMention(final MatchInfo eg, final String relationName) {
        ...
        final String chiSlot0HeadString = chiSlot0Span.isPresent()? chiSlot0Span.get().tokenizedText().utf16CodeUnits() : "NULL";
        final String chiSlot1HeadString = chiSlot1Span.isPresent()? chiSlot1Span.get().tokenizedText().utf16CodeUnits() : "NULL";
        ...
        final String engSlot0HeadString = engSlot0Span.isPresent()? engSlot0Span.get().tokenizedText().utf16CodeUnits() : "NULL";
        final String engSlot1HeadString = engSlot1Span.isPresent()? engSlot1Span.get().tokenizedText().utf16CodeUnits() : "NULL";
    ...
    private static String getSpanText(final Optional<Spanning> span) {
        if(span.isPresent()) {
            return span.get().span().tokenizedText().utf16CodeUnits();
        }
        else {
            return "NULL";
        }
