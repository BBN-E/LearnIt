...
import com.bbn.akbc.neolearnit.modules.MappingExtractionModule;
...
public class LabelExtractor {
    /*
    This class will try to mimic InstanceExtractor's behavior, plus introducting a new concept "LabelExtractor", which intend to introduce another procedure over
    extracted mappings
     */
    ...
    public static void trueMain(String[] args) throws IOException {
        ...
        if (LearnItConfig.optionalParamTrue("bilingual")) {
            ...
            // attach
            // - LearnItExtractorLabelObserver: always label Symbol("Cause") (LearnItObservation/LearnItPattern)
        ...
        System.out.println("In Mapping Extraction stage");
        MappingExtractionModule module = new MappingExtractionModule(info);
        MonolingualDocTheoryInstanceLoader loader = module.getDocTheoryLoader(target);
        LoaderUtils.loadFileList(new File(doclist), loader);
        info = module.getInformationForScoring();
        System.out.println("Out Mapping Extraction stage");

//        PrintDocIdToSentenceIdToInstanceIdentifier.run(info, doclist, target);
