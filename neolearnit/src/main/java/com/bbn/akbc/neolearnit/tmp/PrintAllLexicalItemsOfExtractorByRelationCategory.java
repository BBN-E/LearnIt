...
public class PrintAllLexicalItemsOfExtractorByRelationCategory {
    public static void main(String[] args)throws Exception{
        ...
        for(String targetName:childrenNodeMap.keySet()){
            PrintWriter writer = new PrintWriter("/nfs/ld100/u10/jcai/new_7_23_2020/Hume/resource/causal_relation_word_lists/" + targetName + ".txt", "UTF-8");
                        ...
                        for(Symbol symbol: patternPatternScoreObjectWithScore.getObject().getLexicalItems()){
                            if (symbol.toString().endsWith("[0]") || symbol.toString().endsWith("[1]")) {
                                ...
                                if(symbol.asString().length()>0){
                                    writer.println(symbol.asString());
                                }
                            }
