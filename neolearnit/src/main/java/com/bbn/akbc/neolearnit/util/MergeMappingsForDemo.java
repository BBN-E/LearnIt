...
import com.bbn.akbc.neolearnit.mappings.filters.BadSerifPatternFilter;
...
public class MergeMappingsForDemo {
...
//                    int cnt = existingLearnItPattern.getOrDefault(learnitPattern,0);
                Collection<InstanceIdentifier> instanceIdentifiers = mappings.getInstancesForPattern(learnitPattern);
                boolean isUnaryPattern = false;
                for(InstanceIdentifier instanceIdentifier:instanceIdentifiers){
                    if(instanceIdentifier.isUnaryInstanceIdentifier()){
                        isUnaryPattern = true;
                    }
                    else{
                        isUnaryPattern = false;
                    }
                    break;
                }
                if(isUnaryPattern && BadSerifPatternFilter.isBadUnaryEventSerifPattern((SerifPattern) learnitPattern)){
                    continue;
                }
                if(!isUnaryPattern && BadSerifPatternFilter.isBadBinaryEventEventSerifPattern((SerifPattern) learnitPattern)){
                    continue;
                }
                for(InstanceIdentifier instanceIdentifier: instanceIdentifiers){
//                        if(cnt > 500)break;
