...
public class Target {
	...
	public boolean isUnaryTarget(){
    	for(MatchConstraint matchConstraint:constraints){
    		if(matchConstraint instanceof EmptySlotConstraint)return true;
		}
		return false;
	}
	...
	public boolean isEventTarget(){
    	for (MatchConstraint matchConstraint:constraints){
    		if(matchConstraint instanceof EventMentionOnlyConstraint)return true;
		}
		return false;
	}
...
//		System.out.print(match.toString()+"Matched");
	...
	public boolean validInstance(InstanceIdentifier id, Collection<Seed> seeds) {
		...
		for (MatchConstraint c : constraints) {
			if (!c.valid(id, seeds,this)) {
				return false;
			}
		}

		for (TargetSlot slot : slots) {
			if (!slot.validInstance(id, seeds)) {
				return false;
			}
		}
	...
	public void serialize(String outfile) throws IOException {
        File parentDir = new File(String.format("%s/inputs/targets/json",LearnItConfig.get("learnit_root")));
        if (!parentDir.exists()) {
            parentDir.mkdir();
        }
        StorageUtils.serialize(new File(outfile), this, false);
	}
